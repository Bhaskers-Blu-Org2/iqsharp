 ...
 */
const processInstructions = (instructions: Instruction[], registers: RegisterMap)
    : { metadataList: Metadata[], svgWidth: number } => {

    // Group instructions based on registers
    const groupedOps: number[][] = _groupOperations(instructions, registers);
    ...
    const metadataList: Metadata[] = opsMetadata.flat().filter(({ type }) => type != GateType.Null);
 ...
 */
const _groupOperations = (instructions: Instruction[], registers: RegisterMap): number[][] => {
    ...
    instructions.forEach(({ targets, controls }, instrIdx) => {
 ...
 */
const _opToMetadata = (op: Instruction | null, registers: RegisterMap): Metadata => {
    ...
    if (children != null) {
        ...
        let childrenInstrs = processInstructions(children[0], registers);
